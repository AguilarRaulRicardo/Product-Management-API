...
def create_product(request):
    try:
        ...
        print("error1")
        return Response(
            {"Error": "Missing Token Error"}, status=status.HTTP_400_BAD_REQUEST
        )
    except AttributeError:
        print("error2")
        return Response(
            {"Error": "Missing Authorization Error"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def list_product(request, id):
    try:
        ...
        return Response(
            {"product": "product does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    try:
        ...
        if staff:
            ...
            if serializer.is_valid():
                ...
                return Response(
                    {"category": serializer.data}, status=status.HTTP_200_OK
                )
...
def stock_decrement(request, pk):
    try:
        ...
        if key:
            product = Product.objects.get(id=pk)  # pyright: ignore
            product.stock = product.stock - request.data["quantity"]
            serializer = ProductSerializer(
                product, data={"stock": product.stock}, partial=True
            )
            if serializer.is_valid():
                serializer.save()

                return Response(
                    {"stock updated": serializer.data}, status=status.HTTP_200_OK
                )
...
def update_brand(request, pk):
    try:
        ...
        staff, admin = validate_staff(token)
        if staff:
            brand = Brand.objects.get(id=pk)  # pyright: ignore
...
def update_review(request, pk):
    try:
        ...
        if token:
            try:
                review = Review.objects.get(id=pk, user_id=user)  # pyright: ignore
                ...
                if serializer.is_valid():
                    ...
                    return Response(
                        {"review": serializer.data}, status=status.HTTP_200_OK
                    )
...
def delete_product(request, pk):
    try:
        ...
        if staff:
            ...
            product.delete()
            return Response(
                {"Deleted Product id": [name, id]}, status=status.HTTP_200_OK
            )
        return Response({"Error": "unauthorized"}, status=status.HTTP_401_UNAUTHORIZED)
    except IndexError:
        return Response(
    ...
    try:
        ...
        if staff:
            try:
                ...
                id = review.id
                return Response(
                    {"Deleted review": [name, id]}, status=status.HTTP_200_OK
                )
            except Review.DoesNotExist:
                return Response(
...
def search_product(request):
    ...
    if serializer.is_valid():
        ...
        if "name" in datos:
            query["name__contains"] = serializer.data["name"]
        if "max_price" in datos and "min_price" in datos:
            query["price__lt"] = serializer.data["max_price"]
            query["price__gt"] = serializer.data["min_price"]
        if "category" in datos:
            query["category__exact"] = serializer.data["category"]

        if "brand" in datos:
            query["brand__exact"] = serializer.data["brand"]

        product = Product.objects.filter(**query)

        if product == 0:
            return Response(
                {"Error": "Product dont exist"}, status=status.HTTP_404_NOT_FOUND
            )
        else:
